...
import argparse
import cv2
from models.feature_predictor_adjustment import FeaturePredictorAdj as FeaturePredictor

from collections import OrderedDict
import random
import gin 
from absl import app, flags
from dataset.Loader2 import build_trainloader, build_testloader
...
def evaluation(model, test_loader, output_dir, output_gt, compare_with_pseudo, 
    ...
    with torch.no_grad():
      ...
      for test_batch in tqdm(test_loader): #A scene at a time
        test_batch_gs = gpu_utils.move_to_device([data['gs_params'] for data in test_batch],"cuda")
        test_batch_cameras = gpu_utils.move_to_device([data['cameras'] for data in test_batch],"cuda")
        test_batch_images = gpu_utils.move_to_device([data['images'] for data in test_batch],"cuda")
      ...
      metric_computer.write_to_file(os.path.join(output_dir, f'metrics.rank{0}.json'))
      if compare_with_input:
        metric_computer_input.write_to_file(os.path.join(output_dir, f'metrics_input.rank{0}.json'))
    model.train()

    num_images = torch.tensor([num_images]).to("cuda")
    num_scenes = torch.tensor([num_scenes]).to("cuda")
    for key in metrics:
...
def training(
    ...
    for step in tqdm(range(resume_from_step*accumulate_step, total_steps*accumulate_step)):
      step_consider_accum = step//accumulate_step
      batch = next(train_iterator)

      batch_gs = gpu_utils.move_to_device([data['gs_params'] for data in batch],"cuda")
      batch_cameras = gpu_utils.move_to_device([data['cameras'] for data in batch],"cuda")
      batch_images = gpu_utils.move_to_device([data['images'] for data in batch],"cuda")
    ...
    ########### NOTE: original one time forward ###########
      with torch.cuda.amp.autocast(enabled=enable_amp):
          out_batch_gs = model(**forward_kwargs)
    #######################################################
    ############## NOTE: try batch forward ################
    #   with torch.cuda.amp.autocast(enabled=enable_amp):
    #       num = batch_gs[0]['means'].shape[0]
    #       batch_size = 40000
    #       out_gs = {}
    #       for i in range(0, num // batch_size + 1):
    #         input_gs = {}
    #         for key in batch_gs[0]:
    #           input_gs[key] = batch_gs[0][key][i*batch_size:(i+1)*batch_size]
    #         forward_kwargs = {'batch_normalized_gs': [input_gs], 'batch_scene_idx': [batch_scene_idx]}
    #         pred_gs = model(**forward_kwargs)
    #         for key in pred_gs[0]: 
    #           if key not in out_gs:
    #             out_gs[key] = []
    #           out_gs[key].append(pred_gs[0][key])
    #       if num > (i+1)*batch_size:
    #         input_gs = {}
    #         for key in batch_gs[0]:
    #           input_gs[key] = batch_gs[0][key][(i+1)*batch_size:]
    #         forward_kwargs = {'batch_normalized_gs': [input_gs], 'batch_scene_idx': [batch_scene_idx]}
    #         pred_gs = model(**forward_kwargs)
    #         for key in pred_gs[0]: 
    #           if key not in out_gs:
    ...
    #           out_gs[key].append(pred_gs[0][key])
    #       for key in out_gs:
    #         out_gs[key] = torch.cat(out_gs[key], dim=0)
    #       out_batch_gs = [out_gs]
    #######################################################
      ...
      if step_consider_accum < pretrain_steps:
        ...
        for ii, (out_gs, in_gs) in enumerate(zip(out_batch_gs, batch_gs)):
          ...
          for key in pseudo_target:
            target = pseudo_target[key].to("cuda")
            pred = out_gs[key]
        ...
        optimizer, scheduler = optimizer_['pretrain'], scheduler_['pretrain']
          ...
          for out_gs, cameras, images, in_gs in zip(out_batch_gs, batch_cameras, batch_images, batch_gs):
            ######### NOTE: combine before tarining #########
            combine_gs = {}
            for key in out_gs:
                combine_gs[key] = torch.cat([out_gs[key], in_gs[key]], dim=0)
            pred_imgs, _ = gs_utils.rasterize_gaussians_to_multiimgs(combine_gs, cameras) #a List
            #################################################
      ...
      if step_consider_accum % 100 == 0:
        ...
        with torch.no_grad():
          imgs, _ = gs_utils.rasterize_gaussians_to_multiimgs(
            gpu_utils.move_to_device(out_batch_gs[0], device="cuda"), batch_cameras[0])
          imgs = [(im*255).cpu().numpy().astype(np.uint8) for im in imgs]
        grid = make_grid(imgs)
        grid = cv2.cvtColor(grid, cv2.COLOR_RGB2BGR)
        cv2.imwrite(os.path.join(output_dir, f'train/{step_consider_accum:08d}_pred-rank{0}.png'), grid)
    ...
    #     torch.save(model.state_dict(), os.path.join(output_dir, f'checkpoints/model_{step_consider_accum:08d}.pth'))
